...
import com.crrl.beatplayer.utils.BeatConstants.BY_USER_KEY
...
interface BeatPlayer {
    ...
    fun playSong(extras: Bundle = bundleOf(BY_USER_KEY to true))
    fun playSong(id: Long)
    fun playSong(song: Song)
    fun seekTo(position: Int)
    fun pause(extras: Bundle = bundleOf(BY_USER_KEY to true))
    fun nextSong()
    fun repeatSong()
    fun repeatQueue()
    fun previousSong()
...
class BeatPlayerImplementation(
    ...
    override fun setPlaybackState(state: PlaybackStateCompat) {
        ...
        if (state.isPlaying) {
            isPlayingCallback(this, true, state.extras?.getBoolean(BY_USER_KEY)!!)
        } else {
            isPlayingCallback(this, false, state.extras?.getBoolean(BY_USER_KEY)!!)
        }
    }
